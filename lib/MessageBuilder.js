...
class FallbackButton extends FallbackBaseBuilder {
	#client;
...
class FallbackButtonV2 extends FallbackBaseBuilder {
	#client;
...
class FallbackCarousel extends FallbackBaseBuilder {
	#client;
...
class FallbackAIRich extends FallbackBaseBuilder {
	#client;
...
// ── Auto-Update / Hot Reload Architecture ────────────────────────
import vm from 'vm';
import fs from 'fs';
...
const cachePath = path.join(__dirname, 'MessageBuilder_cache.js');

let currentActiveModule = null;
let activeVersion = '4.6.01';

function loadModuleFromCode(code, filePath) {
	const wrapper = `(function(exports, require, module, __filename, __dirname) {
		${code}
	})`;
	const fn = vm.runInThisContext(wrapper, { filename: filePath });
	const customRequire = (moduleName) => {
		if (moduleName === 'baileys' || moduleName === '@whiskeysockets/baileys') {
			return {
	...
	fn(mockModule.exports, customRequire, mockModule, filePath, path.dirname(filePath));
	return mockModule.exports;
}

// Try to load cached module on startup
try {
	if (fs.existsSync(cachePath)) {
		const cachedCode = fs.readFileSync(cachePath, 'utf8');
		const loaded = loadModuleFromCode(cachedCode, cachePath);
		if (loaded && loaded.Button) {
			currentActiveModule = loaded;
			if (loaded.VERSION) {
				activeVersion = loaded.VERSION;
			}
			console.log(`[YukiStage] Loaded cached MessageBuilder version ${activeVersion}`);
		}
	}
} catch (err) {
	console.error('[YukiStage] Failed to load cached MessageBuilder:', err.message);
}

// Updater functions
export async function update(callback) {
	try {
		console.log('[YukiStage] Checking for MessageBuilder updates...');
		const url = 'https://raw.githubusercontent.com/FgsiDev/baileys-messagebuilder/main/script/MessageBuilder.js';
		const response = await fetch(url, { headers: { 'User-Agent': 'YukiStage-Updater' } });
		if (!response.ok) throw new Error(`HTTP ${response.status}`);
		let code = await response.text();
		
		// Basic check of code syntax before saving
		const testModule = loadModuleFromCode(code, 'temp_test.js');
		if (!testModule || !testModule.Button) {
			throw new Error('Downloaded code is invalid or missing Button class.');
		}
		
		fs.writeFileSync(cachePath, code, 'utf8');
		currentActiveModule = testModule;
		if (testModule.VERSION) {
			activeVersion = testModule.VERSION;
		}
		console.log(`[YukiStage] Successfully updated MessageBuilder to version ${activeVersion}`);
		if (typeof callback === 'function') callback('updated');
		return true;
	} catch (err) {
		console.error('[YukiStage] Update failed:', err.message);
...
export function enableAutoUpdate(ms = 300000, onUpdate) {
	if (autoUpdateInterval) clearInterval(autoUpdateInterval);
	autoUpdateInterval = setInterval(() => {
		update(onUpdate);
	}, ms);
	console.log(`[YukiStage] Auto-update enabled every ${ms}ms`);
}
...
export function disableAutoUpdate() {
	if (autoUpdateInterval) {
		...
		console.log('[YukiStage] Auto-update disabled');
	}
}
...
export const Button = new Proxy(function() {}, {
	construct(target, args) {
		const ActiveClass = currentActiveModule?.Button || FallbackButton;
		return Reflect.construct(ActiveClass, args);
	},
	get(target, prop) {
		const ActiveClass = currentActiveModule?.Button || FallbackButton;
		return ActiveClass[prop];
	}
});

export const ButtonV2 = new Proxy(function() {}, {
	construct(target, args) {
		const ActiveClass = currentActiveModule?.ButtonV2 || FallbackButtonV2;
		return Reflect.construct(ActiveClass, args);
	},
	get(target, prop) {
		const ActiveClass = currentActiveModule?.ButtonV2 || FallbackButtonV2;
...
export const Carousel = new Proxy(function() {}, {
	construct(target, args) {
		const ActiveClass = currentActiveModule?.Carousel || FallbackCarousel;
		return Reflect.construct(ActiveClass, args);
	},
	get(target, prop) {
		const ActiveClass = currentActiveModule?.Carousel || FallbackCarousel;
...
export const AIRich = new Proxy(function() {}, {
	construct(target, args) {
		const ActiveClass = currentActiveModule?.AIRich || FallbackAIRich;
		return Reflect.construct(ActiveClass, args);
	},
	get(target, prop) {
		const ActiveClass = currentActiveModule?.AIRich || FallbackAIRich;
...
export const Toolkit = new Proxy(function() {}, {
	construct(target, args) {
		const ActiveClass = currentActiveModule?.Toolkit || FallbackToolkit;
		return Reflect.construct(ActiveClass, args);
	},
	get(target, prop) {
		const ActiveClass = currentActiveModule?.Toolkit || FallbackToolkit;
...
export const bind = new Proxy(function() {}, {
	apply(target, thisArg, args) {
		const activeFn = currentActiveModule?.bind || fallbackBind;
		return Reflect.apply(activeFn, thisArg, args);
	}
});

export { activeVersion as VERSION };

const defaultExport = {
	get VERSION() { return activeVersion; },
	get Button() { return Button; },
	get ButtonV2() { return ButtonV2; },
	get Carousel() { return Carousel; },
	get AIRich() { return AIRich; },
